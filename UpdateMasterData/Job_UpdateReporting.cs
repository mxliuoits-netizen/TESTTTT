...
namespace UpdateMasterData
{
    ...
    /// 彙報關係 JOB
    /// </summary>
    public class Job_UpdateReporting : BaseJob
    {
        #region 設定全域變數
        SFTPClient sftp;
        ...
        public Job_UpdateReporting(string jobName, Logger logger, params string[] args)
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                string sql= "UPDATE " + settingDictionary["Table"] + " SET REPORTDATE = '" + this.jobVersion + "'";
                da.DapperExecute(sql);
                #endregion

                #region 更新[匯報關係]資料
                this.CurrentJobStepText = "更新匯報關係資料開始";
                DateTime jobVersionDate = DateTime.ParseExact(this.jobVersion, BaseJob.VER_DATE_FORMAT, null);
                ReportingService reportingService = new ReportingService(da, jobMetaData);
                reportingService.Import(jobVersionDate);
                reportingService.RebuildBaseTeamOwner(jobVersionDate);
                this.WriteLog("更新匯報關係資料結束", false);
                #endregion
